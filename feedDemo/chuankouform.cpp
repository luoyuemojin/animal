...
#include "ui_chuankouform.h"

chuankouForm::chuankouForm(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::chuankouForm)
{
...
chuankouForm::~chuankouForm()
{
    delete ui;
}

void chuankouForm::on_OpenSerialButton_clicked()//打开串口
{
    bool ok;
    BYTE sUID[8];//uid 为8位
    DWORD tag=0;
...
void chuankouForm::on_closeSerialButton_clicked()//关闭串口
{
    //恢复设置使能
    ui->PortBox->setEnabled(true);
    ui->BaudBox->setEnabled(true);
